...
def get_decoder_try(embedding, skip_connections, n_filters=16, dropout=0.5, batchnorm=True, channels_out=3, out_clf=5):
    # load skip connections with shape: (16, 128), (32, 128), (64, 128), (128, 64), (256, 32), (512, 16))
    c5, c4, c3, c2, c1, c0, in_enc = skip_connections[0], skip_connections[1], skip_connections[2], skip_connections[3], \
                                     skip_connections[4], skip_connections[5], skip_connections[-1]
...
concat = decoder.get_layer('c9_concat')
...
def get_RAEwSCwWSwIwCLF_compiled(weights=None, length_seq_in=3, lr = 0.001, grad_clip=1., loss_weights={'predicted_frames':1., 'predicted_emb':1.}, 
                                 dropout_enc=0.05, dropout_dec=0.05, b_norm_enc=True, b_norm_dec=True,
                                 general_loss_weights={'Concat_predicted_frames':1., 'softmax_clf':.4}):
