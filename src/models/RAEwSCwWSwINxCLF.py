...
def conv2d_block(input_tensor, n_filters, kernel_size=3, batchnorm=True, name='', batchnorm_implement=True):
    ...
    if batchnorm_implement:
        x = BatchNormalization(trainable=batchnorm, name=name+'_BN1')(x)
    x = Activation(ACTIVATION)(x)
    # second layer
    x = Conv2D(filters=n_filters, kernel_size=(kernel_size, kernel_size), kernel_initializer="he_normal",
               padding="same", name=name+'_conv2')(x)
    if batchnorm_implement:
        x = BatchNormalization(trainable=batchnorm, name=name+'_BN2')(x)
    x = Activation(ACTIVATION)(x)
    return x
...
def get_decoder(embedding, skip_connections, n_filters=16, dropout=0.5, batchnorm=True, channels_out=3, out_clf=5):
    ...
    clf = conv2d_block(c9, n_filters=out_clf, kernel_size=3, batchnorm=batchnorm, name='clf_layer')
    ...
    return Model(inputs=[embedding, c5, c4, c3, c2, c1, c0, in_enc], outputs=[outputs, clf], name='decoder')
...
def get_encoder_decoder_w_input(embedding_size=2048, im_height=495, im_width=436, channels=3, 
    ...
    # get encoder/decoder models to us them as a layers. 
    # Perhaps its weights could be loaded here?
...
def get_RAEwSCwWSwIwCLF_compiled(weights=None, length_seq_in=3, lr = 0.001, grad_clip=1., loss_weights={'predicted_frames':1., 'predicted_emb':1.}, 
                                 dropout_enc=0.05, dropout_dec=0.05, b_norm_enc=True, b_norm_dec=True,
                                 general_loss_weights={'Concat_predicted_frames':1., 'softmax_clf':.0}):
    """ define and compile the model """
    
    params = {'units_before_recurrent': 2048, 'gru_enc_1': 256, 'gru_enc_2': 128, 'gru_dec_1': 128, 'gru_dec_2': 256, "embed_weather": 8}
    model, embeddings, future_embeddings = get_RAEwSC_and_ExogenousVars_w_input_w_clf(params, length_seq_in=length_seq_in, 
                                           dropout_enc=dropout_enc, dropout_dec=dropout_dec, 
                                           b_norm_enc=b_norm_enc, b_norm_dec=b_norm_dec)
    start_name_layer_to_freeze = 'clf_layer'
    for i, layer in enumerate(model.get_layer("decoder").layers):
        if start_name_layer_to_freeze in layer.name:
    ...
    model.compile(optimizer=optimizer,
                  loss={'Concat_predicted_frames': loss_with_latent_term_2(future_embeddings, embeddings, loss_weights),
                        'softmax_clf': losses.sparse_categorical_crossentropy},
                  metrics={'Concat_predicted_frames': ['mse', get_recurrent_embedding_loss(future_embeddings, embeddings, loss_weights)], 
...
· accuracy as metric can't be used with sparse_Categorical_cross_entropy
